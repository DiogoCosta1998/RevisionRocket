...
def getQuestions():
    if request.method == "POST":
        ...
        try:
            connection = engine.connect()
            questions = connection.execute("SELECT QuestionsAnswers.question_id, QuestionsAnswers.question_name, QuestionsAnswers.option_a, QuestionsAnswers.option_b, QuestionsAnswers.option_c, QuestionsAnswers.option_d, QuestionsAnswers.correct_answer, QuestionsAnswers.answer_reason FROM QuestionsAnswers INNER JOIN Topics ON Topics.topic_id=QuestionsAnswers.topic_id INNER JOIN Subjects ON Subjects.subject_id=Topics.subject_id AND Subjects.qualification_level='" + qualificationSelected + "' AND Subjects.subject_name='" + subjectSelected + "' WHERE Topics.topic_name='" + topicSelected + "'")
            connection.close()
...
@app.route('/getStudentAverage', methods=["POST"])
def getStudentAverage():
    if request.method == "POST":
        ...
        try:
            connection = engine.connect()
            topicAverage = connection.execute("SELECT AverageTopicResults.average, Subjects.subject_name, Subjects.qualification_level, Topics.topic_name FROM AverageTopicResults INNER JOIN Students ON AverageTopicResults.email_address=Students.email_address INNER JOIN Topics ON AverageTopicResults.topic_id=Topics.topic_id INNER JOIN Subjects ON Subjects.subject_id=Topics.subject_id WHERE Students.name='" + studentName + "'")
            connection.close()

        except Exception as e:
            return str(e)

    return json.dumps([dict(average) for average in topicAverage], default=str)
...
def uploadRevisionMaterials():
    if request.method == "POST":
        ...
        topicSelected = request.form["topic"]
        materialType = request.form["material_type"]
        materialLink = request.form["material_type"]
        #print('DEBUG: Method= ' + subjectSelected + " " + qualificationSelected, file=sys.stderr)

        try:
            connection = engine.connect()
            revisionMaterials = connection.execute("SELECT Subjects.subject_name, Topics.topic_name, RevisionMaterials.material_id, RevisionMaterials.material_type FROM Subjects INNER JOIN Topics ON Subjects.subject_id=Topics.subject_id AND Subjects.qualification_level='" + qualificationSelected + "' AND Subjects.subject_name='" + subjectSelected + "' INNER JOIN RevisionMaterials ON RevisionMaterials.topic_id=Topics.topic_id AND Topics.topic_name='" + topicSelected + "' AND RevisionMaterials.material_type='" + materialType + "'")
            connection.close()
...
def submitQuestion():
    if request.method == "POST":
        ...
        questionReason = request.form["questionReason"]

        try:
            connection = engine.connect()
            questionEntry = connection.execute("INSERT INTO QuestionsAnswers(question_name, option_a, option_b, option_c, option_d, correct_answer, answer_reason, topic_id) SELECT '" + questionText + "', '" + optionA + "', '" + optionB + "', '" + optionC + "', '" + optionD + "', '" + correctOption + "', '" + questionReason + "', Topics.topic_id FROM Subjects INNER JOIN Topics ON Subjects.subject_id=Topics.subject_id AND Subjects.qualification_level='" + qualificationLevelSelected + "' AND Subjects.subject_name='" + subjectSelected + "' AND Topics.topic_name='" + topicSelected + "'")
            connection.close()
    ...
    return json.dumps([dict(question) for question in questionEntry])

@app.route('/updateQuestion', methods=["POST"])
def updateQuestion():
    if request.method == "POST":
        questionId = request.form["question_id"]
        questionText = request.form["question_text"]
        optionA = request.form["option_a"]
        optionB = request.form["option_b"]
        optionC = request.form["option_c"]
        optionD = request.form["option_d"]
        correctOption = request.form["correct_option"]
        questionReason = request.form["answer_reason"]

        try:
            connection = engine.connect()
            changesSubmitted = connection.execute("UPDATE QuestionsAnswers SET question_name='" + questionText + "', option_a='" + optionA + "', option_b='" + optionB + "', option_c='" + optionC + "', option_d='" + optionD + "', correct_answer='" + correctOption + "', answer_reason='" + questionReason + "' WHERE question_id='" + questionId + "'")
            connection.close()
    ...
    return json.dumps([dict(changes) for changes in changesSubmitted])

@app.route('/getTeacherData', methods=["GET"])
def getTeacherData():
...
def getStudents():
    if request.method == "GET":
        try:
            connection = engine.connect()
            students = connection.execute("SELECT DISTINCT Students.name FROM Students INNER JOIN QuizResults ON Students.email_address=QuizResults.email_address")
            connection.close()
